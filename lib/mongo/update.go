...
func UpdateOne(collectionName string, filter, data types.M, option *options.FindOneAndUpdateOptions) error {
...
func UpdateInstance(filter, data types.M) error {
...
func UpsertInstance(filter, data types.M) error {
...
func UpdateUser(filter, data types.M) error {
...
// or inserts it if the corresponding document doesn't exist
func UpsertUser(filter, data types.M) error {
...
func UpdateMany(collectionName string, filter, data types.M) (interface{}, error) {
	collection := link.Collection(collectionName)
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
...
func UpdateInstances(filter, data types.M) (interface{}, error) {
