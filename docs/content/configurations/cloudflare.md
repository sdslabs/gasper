...
???example
    If the domain parameter's value is `sdslabs.co` and you have created an application named **foo**, then an entry will be created in cloudflare (if plugin enabled) with the domain name `foo-app-gasper.sdslabs.co`
...
!!!warning
    The generated token must have the permissions **ZONE:ZONE:READ**, **ZONE:ZONE:EDIT**, **ZONE:DNS:EDIT** and **ACCOUNT:ACCOUNT SETTINGS:READ** in order for this plugin to work
